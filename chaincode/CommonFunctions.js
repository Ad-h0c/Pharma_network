...
async function registerCompany(
  ...
  if ("consumerMSP" == ctx.clientIdentity.mspId) {
    throw new Error(
      "You are not authorized to perform this operation : Your Organization is : " +
        ctx.clientIdentity.mspId
    );
  }
  ...
  if (companySearchResults.length > 0) {
    throw new Error(
      "Invalid COMPANY Details. Another company with this CRN already exists."
    );
  }
  ...
  if (dataBuffer.toString()) {
    throw new Error(
      "Invalid COMPANY Details. Another company with this CRN & Name already exists."
    );
  }
...
async function createPO(ctx, buyerCRN, sellerCRN, drugName, quantity) {
  ...
  if (
    parseInt(sellerCompanyInfo.hierarchyKey) + 1 !=
    parseInt(buyerCompanyInfo.hierarchyKey)
  ) {
    throw new Error(
      "You can't purchase directly from " +
        sellerCompanyInfo.organisationRole +
        " sellerCompanyInfo.hierarchyKey : " +
        sellerCompanyInfo.hierarchyKey +
        " buyerCompanyInfo.hierarchyKey : " +
        buyerCompanyInfo.hierarchyKey
    );
  }

  // Create a new composite key for the new Purchase Order
  const poID = ctx.stub.createCompositeKey("org.pharma-network.pharmanet.po", [
    buyerCRN,
    drugName,
  ]);
...
async function createShipment(
  ...
  let transporterCompanySearchResults = await searchCompanyByCRN(
    ctx,
    transporterCRN
  );
  ...
  let poDataBuffer = await ctx.stub
    .getState(poID)
    .catch((err) => console.log(err));
  if (!poDataBuffer.toString()) {
    throw new Error(
      "Invalid operation. No PO raised for BUYER :" +
        buyerCRN +
        ", DRUG : " +
        drugName
    );
  ...
  for (var i = 0; i < drugSearchResults.length; i++) {
    ...
    if (
      drugFoundWithSameName.owner === poDetails.additionalInfo.sellerCompanyID
    ) {
      drugFoundWithSeller = true;
  ...
  if (drugAssetsToBeAttached.length < poDetails.quantity) {
    throw new Error(
      "At this moment, SELLER does not have sufficient stock of DRUG : " +
        drugName
    );
  }
  // Create a new composite key for the new SHIPMENT Order
  const shipmentID = ctx.stub.createCompositeKey(
...
async function updateShipment(ctx, buyerCRN, drugName, transporterCRN) {
  ...
  let shipmentoDataBuffer = await ctx.stub
    .getState(shipmentID)
    .catch((err) => console.log(err));
  if (!shipmentoDataBuffer.toString()) {
    throw new Error(
      "Invalid operation. Can't find any SHIPMENT with specified BUYER & DRUG"
    );
  }
  ...
  for (var i = 0; i < shipmentObject.assets.length; i++) {
    const drugObjectDataBuffer = await ctx.stub
      .getState(shipmentObject.assets[i])
      .catch((err) => console.log(err));
    ...
    await ctx.stub.putState(
      drugObject.productID,
      Buffer.from(JSON.stringify(drugObject))
    );
  }
...
async function retailDrug(
  ...
  let drugObjectDataBuffer = await ctx.stub
    .getState(productID)
    .catch((err) => console.log(err));
  if (!drugObjectDataBuffer.toString()) {
    throw new Error(
      "No DRUG available with specified Name & Serial No",
      drugName,
      serialNo
    );
  }
  ...
  if (drugObject.owner != retailerCompanySearchResults[0].companyID) {
    throw new Error(
      "You can't sell this DRUG as you are not the OWNER of this DRUG ",
      drugName,
      serialNo
    );
  }
...
async function viewHistory(ctx, drugName, serialNo) {
  ...
  let historyQueryIterator = await ctx.stub
    .getHistoryForKey(productID)
    .catch((err) => console.log(err));
...
async function searchCompanyByCRN(ctx, companyCRN) {
  const companySearchResultsIterator =
