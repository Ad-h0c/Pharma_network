...
class ManufacturerContract extends Contract {
  ...
  async addDrug(ctx, drugName, serialNo, mfgDate, expDate, companyCRN) {
    ...
    let companySearchResults = await commonFunctions.searchCompanyByCRN(
      ctx,
      companyCRN
    );
    ...
    if (companySearchResults[0].organisationRole != "MANUFACTURER") {
      throw new Error(
        "You are not registered as MANUFACTURER and can't add DRUGs with your license"
      );
    }

    let drugSearchResults = await commonFunctions.searchDrugByName(
      ctx,
      drugName
    );
    ...
    for (var i = 0; i < drugSearchResults.length; i++) {
      ...
      if (drugFoundWithSameName.productID === productID) {
        ...
        throw new Error(
          "You already have a DRUG with same " + drugName + " & " + serialNo
        );
      }
    }
