...
async function registerCompany(orgType, reqPayload) {
  try {
    ...
    const registerCompanyBuffer = await contractInstance.submitTransaction(
      "registerCompany",
      reqPayload.companyCRN,
      reqPayload.companyName,
      reqPayload.location,
      reqPayload.organisationRole
    );
...
async function addDrug(orgType, reqPayload) {
  try {
    const contractInstance = await helper.getContractInstance(orgType);
    const addDrugBuffer = await contractInstance.submitTransaction(
      "addDrug",
      reqPayload.drugName,
      reqPayload.serialNo,
      reqPayload.mfgDate,
...
async function viewDrugCurrentState(orgType, reqPayload) {
  try {
    const contractInstance = await helper.getContractInstance(orgType);
    const viewDrugCurrentStateBuffer = await contractInstance.submitTransaction(
    ...
    console.log(`\n\n ${error} \n\n`);
    throw new Error(
      `Error while invoking viewDrugCurrentState : ${error.message}`
    );
  } finally {
    // Disconnect from the fabric gateway
    helper.disconnect();
...
async function viewHistory(orgType, reqPayload) {
  try {
    const contractInstance = await helper.getContractInstance(orgType);
    const viewHistoryBuffer = await contractInstance.submitTransaction(
      "viewHistory",
      reqPayload.drugName,
      reqPayload.serialNo
    );
...
async function createPO(orgType, reqPayload) {
  try {
    ...
    const createPOBuffer = await contractInstance.submitTransaction(
      "createPO",
      reqPayload.buyerCRN,
      reqPayload.sellerCRN,
      reqPayload.drugName,
      reqPayload.quantity
    );
...
async function createShipment(orgType, reqPayload) {
  try {
    const contractInstance = await helper.getContractInstance(orgType);
    const createShipmentBuffer = await contractInstance.submitTransaction(
      "createShipment",
      reqPayload.buyerCRN,
      reqPayload.drugName,
      reqPayload.listOfAssets,
      reqPayload.transporterCRN
    );
...
async function updateShipment(orgType, reqPayload) {
  try {
    const contractInstance = await helper.getContractInstance(orgType);
    const updateShipmentBuffer = await contractInstance.submitTransaction(
      "updateShipment",
      reqPayload.buyerCRN,
      reqPayload.drugName,
      reqPayload.transporterCRN
    ...
    console.log(
      "\n\n.....updateShipment Transaction Complete!",
      shipmentObject
    );
    return shipmentObject;
  } catch (error) {
...
async function retailDrug(orgType, reqPayload) {
  try {
    const contractInstance = await helper.getContractInstance(orgType);
    const retailDrugBuffer = await contractInstance.submitTransaction(
      "retailDrug",
      reqPayload.drugName,
      reqPayload.serialNo,
      reqPayload.retailerCRN,
      reqPayload.customerAadhar
    );
